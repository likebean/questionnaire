...
| POST | /api/fill/:id/submit 或 /api/s/:shortCode/submit | 提交答卷 | Body: { items: [{ questionId, optionIndex?（单选/量表）, optionIndices?（多选）, textValue?, scaleValue? }], durationSeconds? }。选项为题目 JSON 时用下标（0 起）。需登录时带鉴权；匿名时无 user_id。Response: 成功 200，前端展示感谢语 |
