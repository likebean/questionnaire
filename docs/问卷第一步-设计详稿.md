...
**config 结构**：与 options 及多选/填空/量表参数**统一为一个 JSON 字段** `config`，按题型使用其中部分字段。

| 字段 | 类型 | 说明 |
|------|------|------|
| **options** | array | 单选/多选：选项列表，项为 `{ sortOrder, label, isOther?, allowFill? }`；量表可不设或为空 |
| **minChoices** | int | 多选：最少选几项，可选 |
| **maxChoices** | int | 多选：最多选几项，可选 |
| **placeholder** | string | 填空：占位提示 |
| **multiline** | boolean | 填空：true=多行，false=单行 |
| **scaleMin** | int | 量表：一般为 1 |
| **scaleMax** | int | 量表：如 5/7/10 |
| **scaleLeftLabel** | string | 量表：左端点文案 |
| **scaleRightLabel** | string | 量表：右端点文案 |

示例（单选题）：`{ "options": [{ "sortOrder": 0, "label": "选项A", "isOther": false, "allowFill": false }, { "sortOrder": 1, "label": "其他", "isOther": true, "allowFill": true }] }`  
示例（多选题）：`{ "options": [...], "minChoices": 1, "maxChoices": 3 }`  
示例（填空题）：`{ "placeholder": "请输入", "multiline": false }`  
示例（量表题）：`{ "scaleMin": 1, "scaleMax": 5, "scaleLeftLabel": "非常不同意", "scaleRightLabel": "非常同意" }`
